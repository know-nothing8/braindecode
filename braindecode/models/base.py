...
import torch as th
from braindecode.experiments.monitors import LossMonitor, MisclassMonitor, RuntimeMonitor, CroppedTrialMisclassMonitor, compute_trial_labels_from_crop_preds, compute_pred_labels_from_trial_preds, compute_preds_per_trial_from_crops
...
class BaseModel(object):
    def cuda(self):
        ...
        assert (not self.compiled), "Call cuda before compiling model, otherwise optimization will not work"
    ...
    def compile(self, loss, optimizer, extra_monitors=None, cropped=False, iterator_seed=0, ):
        ...
        self._ensure_network_exists()
        if cropped:
            model_already_dense = np.any([hasattr(m, "dilation") and (m.dilation != 1) and (m.dilation) != (1, 1) for m in self.network.modules()])
    ...
    def fit(self, train_X, train_y, epochs, batch_size, input_time_length=None, validation_data=None, model_constraint=None, remember_best_column=None, scheduler=None, log_0_epoch=True, ):
        """
        ...
        batch_size: int input_time_length: int, optional Super crop size, what temporal size is pushed forward through the network, see cropped decoding tuturial.
        ...
        scheduler: 'cosine' or None, optional Whether to use cosine annealing (:class:`.CosineAnnealing`).
        log_0_epoch: bool Whether to compute the metrics once before training as well.
        Returns
        -------
        exp: Underlying braindecode :class:`.
        Experiment`
        """
        ...
        if self.cropped and input_time_length is None:
            raise ValueError("In cropped mode, need to specify input_time_length, which is the number of timesteps that will be pushed through the network in a single pass.")

        train_X = _ensure_float32(train_X)
        if self.cropped:
            self.network.eval()
            test_input = np_to_var(np.ones((1, train_X[0].shape[0], input_time_length) + train_X[0].shape[2:], dtype=np.float32, ))
            ...
            self.iterator = CropsFromTrialsIterator(batch_size=batch_size, input_time_length=input_time_length, n_preds_per_input=n_preds_per_input, seed=self.seed_rng.randint(0, np.iinfo(np.int32).max - 1), )
        else:
            self.iterator = BalancedBatchSizeIterator(batch_size=batch_size, seed=self.seed_rng.randint(0, np.iinfo(np.int32).max - 1), )
        if log_0_epoch:
        ...
        if scheduler is not None:
            ...
            n_updates_per_epoch = sum([1 for _ in self.iterator.get_batches(train_set, shuffle=True)])
            ...
            optimizer = ScheduledOptimizer(scheduler, self.optimizer, schedule_weight_decay=schedule_weight_decay, )
        loss_function = self.loss
        if self.cropped:
            loss_function = lambda outputs, targets: self.loss(th.mean(outputs, dim=2), targets)
    ...
    def evaluate(self, X, y):
        ...
        Returns
        -------
        result: dict             Dictionary with result metrics.
        ...
        for monitor in self.monitors:
            if hasattr(monitor, "last_call_time"):                 monitor.last_call_time = time.time()
        exp = Experiment(self.network, train_set, valid_set, test_set, iterator=self.iterator, loss_function=loss_function, optimizer=self.optimizer, model_constraint=model_constraint, monitors=self.monitors, stop_criterion=stop_criterion, remember_best_column=None, run_after_early_stop=False, cuda=self.is_cuda, log_0_epoch=True, do_early_stop=False, )
        exp.monitor_epoch({"train": train_set})

        result_dict = dict([(key.replace("train_", ""), val) for key, val in dict(exp.epochs_df.iloc[0]).items()])
        return result_dict

    def predict_classes(self, X, threshold_for_binary_case=None, individual_crops=False):
        """
        ...
        X: ndarray
            Input data.
        threshold_for_binary_case: float, optional             In case of a model with single output, the threshold for assigning,
    ...
    def predict_outs(self, X, individual_crops=False):
        ...
        Returns
        -------
            outs_per_trial: 2darray or list of 2darrays                 Network outputs for each trial, optionally for each crop within trial.
        """
        if individual_crops:
            assert self.cropped, "Cropped labels only for cropped decoding"
        X = _ensure_float32(X)
        ...
        if self.cropped:
            outs_per_trial = compute_preds_per_trial_from_crops(all_preds, self.iterator.input_time_length, X)
            if not individual_crops:
