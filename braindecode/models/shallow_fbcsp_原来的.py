...
class ShallowFBCSPNet(BaseModel):
    ...
    def create_network(self):
        ...
        if self.split_first_layer:
            ...
            model.add_module("conv_time", nn.Conv2d(in_channels=1, out_channels=self.n_filters_time, kernel_size=(self.filter_time_length, 1), stride=1, ), )
            model.add_module("conv_spat",nn.Conv2d(in_channels=self.n_filters_time, out_channels=self.n_filters_spat, kernel_size=(1, self.in_chans), stride=1,bias=not self.batch_norm, ), )
            n_filters_conv = self.n_filters_spat
        else:
            # C_im=64, n_f=C_o=40, H_f=25
            model.add_module("conv_time",nn.Conv2d(in_channels=self.in_chans, out_channels=self.n_filters_time, kernel_size=(self.filter_time_length, 1), stride=1,bias=not self.batch_norm, ), )
            n_filters_conv = self.n_filters_time
        if self.batch_norm:
            model.add_module("bnorm", nn.BatchNorm2d(num_features=n_filters_conv, momentum=self.batch_norm_alpha, affine=True), )
        model.add_module("conv_nonlin", Expression(self.conv_nonlin))
        model.add_module("pool",pool_class(kernel_size=(self.pool_time_length, 1), stride=(self.pool_time_stride, 1), ), )
        ...
        model.add_module("conv_classifier",nn.Conv2d(n_filters_conv, self.n_classes, (self.final_conv_length, 1), bias=True, ), )
